...
//
// CHANGE: replace most sqrtf() usage in the hot paths with a fast inverse sqrt (rsqrt).
//  - Avoidance kernel: uses rsqrt for invDist and uses a dsq-based falloff (no dist sqrt).
//  - Normalize in RandDir and clamp_len uses rsqrt as well.
//
// Notes:
//  - FAST_RSQRT is approximate. If you want higher accuracy, run 2 Newton steps or
//    fall back to 1.0f/sqrtf() via the macro.
//  - The dsq-based falloff changes the exact curve slightly vs (1 - dist/r)^2,
//    but visually it’s very close and avoids expensive sqrt.
...
#define PROGRESS_TOLERANCE   0.15f  // Was 0.30
#define STUCK_THRESHOLD      1.5f   // Was 1.0
...
// ---- fast math knobs ----
// 0 = use 1/sqrtf, 1 = use fast rsqrt approximation
#define USE_FAST_RSQRT 1
// Newton steps: 1 is usually fine; 2 is more accurate but a tad slower.
...
#if USE_FAST_RSQRT
    // Handle non-positive to avoid NaNs.
...
static inline Vector2 vclamp_len(Vector2 v, float maxLen) {
    float l2 = vlen2(v);
    float max2 = maxLen * maxLen;
    if (l2 > max2) {
        float invLen = fast_rsqrt(l2);
        return vmul(v, maxLen * invLen);
    }
    return v;
}
                                     ...
                                     bool* outHitNeighborCap)
{
    ...
    // dsq-based falloff constants (approximate (1 - dist/r)^2)
    ...
    for (int oy = -radCells; oy <= radCells; oy++) {
        ...
        for (int ox = -radCells; ox <= radCells; ox++) {
            ...
            for (int t = start; t < end; t++) {
                ...
                // invDist via fast rsqrt
                float invDist = fast_rsqrt(dsq);

                // dsq-based falloff (no sqrt):
                // u = 1 - (dsq / r2)  (instead of 1 - dist/r)
                float u = 1.0f - (dsq * invR2);
                // keep non-negative
                if (u <= 0.0f) continue;
                float strength = u * u;
...
int main(void) {
    ...
    InitWindow(screenW, screenH, "Raylib crowd: separation + bucket grid + accurate profiler");
    SetTargetFPS(60);
    ...
    cam.target = (Vector2){ WORLD_W * 0.5f, screenH * 0.5f };
    ...
    for (int i = 0; i < AGENT_COUNT; i++) {
        ...
        // lastDist needs actual distance (can use sqrt here, it's not the inner kernel)
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < AGENT_COUNT; i++) {
            ...
            if (!avoidanceOn) {
                // We still need actual distance for step clamp; keep sqrt here.
            ...
            // avoidance ON path:
            // Here we can compute invDist with rsqrt and dist as 1/invDist.
            float invDist = fast_rsqrt(dist2);
            // Avoid div by 0
            float dist = (invDist > 0.0f) ? (1.0f / invDist) : 0.0f;
            Vector2 desiredDir = vmul(to, invDist);

            // progress-based stuck detection (needs dist)
        ...
        DrawText("WASD/Arrows: pan | Wheel: zoom | V: avoidance | R: draw | C: cull", 12, 12, 18, RAYWHITE);
        ...
        DrawText(cullDraw ? "Cull: ON" : "Cull: OFF", 12, 84, 18, cullDraw ? GREEN : RED);
        DrawFPS(12, 108);
        ...
        DrawTextShadow(TextFormat("CELL_SIZE=%.1f radCells=%d  scanCap=%d neighCap=%d period=%d  rsqrt=%s",
                                  (float)CELL_SIZE, radCells, (int)AVOID_MAX_SCAN, (int)AVOID_MAX_NEIGHBORS, (int)AVOID_PERIOD,
                                  USE_FAST_RSQRT ? "FAST" : "SQRT"),
